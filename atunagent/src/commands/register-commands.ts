...
import type { AtunAgentState } from '../agent-state';
...
import type { WorkspaceActionResult, WorkspaceTools } from '../workspace-tools';

const SHELL_CONTAINER_COMMAND = 'workbench.view.extension.atunAgentSidebar';
const CHAT_QUERY = '@atun ';

interface RegisterCommandsInput {
...
export async function revealSidebarOnFirstActivation(context: vscode.ExtensionContext): Promise<void> {
...
async function safeOpenNativeChat(hostSupport: HostSupport): Promise<void> {
	try {
...
async function openNativeChat(hostSupport: HostSupport): Promise<void> {
